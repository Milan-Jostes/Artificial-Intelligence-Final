from CustomLanguage.strings_with_arrows import *
import string
import random
DIGITS      = '0123456789'
LETTER      = string.ascii_letters
LETTERS_DIGITS = LETTER + DIGITS
TT_STR      = "STR"
TT_INT      = 'INT'
TT_FLOAT    = 'FLOAT'
TT_KEYWORD  = 'KEYWORD'
...
class Lexer:
    ...
    def roll_dice(self):
        ...
        pos_start = self.pos.copy()
        self.advance()
        return Token(TT_DICE, str(val),pos_start,self.pos)
    ...
    def make_string(self):
        ...
        while self.current_char != None and self.current_char in LETTER:
            ...
            self.advance()
        return Token(TT_STR, str(word), pos_start,self.pos)
...
class Parser:
    ...
    def parse(self):
        ...
        if not res.error and self.current_tok.type != TT_EOF:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Parse: Expected '+', '-', '*', or '\'"))
        return res
    ...
    def factor(self):
        ...
        if tok.type in ((TT_PLUS,TT_MINUS)):
            ...
            return res.success(UnararyOpNode(tok,factor))
...
class Number:
    ...
    def multed_by(self, other):
        if isinstance(other, Number):
            if other.value == 0: return None, RuntimeError("Modulo by zero", self.context)
            return Number(self.value * other.value).set_context(self.context), None

    def dived_by(self, other):
        if isinstance(other, Number):
            if other.value == 0: return None, RuntimeError("Division by zero", self.context)
            return Number(self.value / other.value).set_context(self.context), None
    ...
    def roll_dice(self, other):
        nums = []
        if isinstance(other, Number):
            #value = random.randint(1,other.value)
            for x in range(self.value):
                nums.append(random.randint(1,other.value))

        # return Number(value).set_context(self.context), None
        return Number(nums).set_context(self.context), None
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_PLUS:
            ...
            result, error = left.floor_divide(right)
        elif node.op_tok.type == TT_DICE:
            result, error = left.roll_dice(right)
